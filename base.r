...
d = read.csv("https://opendata.ecdc.europa.eu/covid19/casedistribution/csv", na.strings = "", fileEncoding = "UTF-8-BOM")
## get CFR
...
for(Country in countries) {
  ...
  d1=d[d$countriesAndTerritories==Country,]
...
exportJson <- toJSON(stan_data)
write(exportJson, "test.json")


  stan_data2 = stan_data
  stan_data2$covariate4 = unname(as.matrix(stan_data$covariate4 ))
  stan_data2$deaths = unname(as.matrix(stan_data$deaths ))
  stan_data2$f = unname(as.matrix(stan_data$f ))  
  stan_data2$cases = unname(as.matrix(stan_data$cases))
  
  exportJson <- toJSON(stan_data2)
  write(exportJson, "data/stan_data2.json")
